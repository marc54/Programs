...
int closest(int[], int, int);                                   //Function Prototype6

ifstream infile("Assignment7.2.txt");
ofstream outfile  ("outfile7.2.txt");

int const MAX=100;

int main()
{
    int n, score1[MAX], score2[MAX], score3[MAX],sumscores[MAX], smallest, position, k;
    int  arrval;
   ...
   cout<<"----------------------------------------------";
   cout<<endl<<endl<<"To turn off search option type in any number besides 1, 2, 3, or 4."<<endl<<endl;

    int q;
   cout<<"Please enter an array number ( 1,2,3 or 4) ::>> ";
   cin >> q;
   while ( q==1 || q==2 || q==3 || q==4 )
    {

        cout<< "Please enter value you want to find ::>> ";
        cin>>arrval;

    if (q==1)
        {
        k = closest(score1, arrval, n);
        if (k!=-1)
                {
                cout<<"The position with the closest value to " <<arrval<<" is "<< k <<endl<<endl;
                }
            else
                {
                cout<<" are the closest positions to "<<arrval<<endl<<endl;
                }
        }
    else if (q==2)
        {
            k=closest(score2, arrval, n);
        if (k!=-1)
                {
                cout<<"The position with the closest value to " <<arrval<<" is "<< k <<endl<<endl;
                }
...
//Function6
int closest(int arr[], int searchval, int m )
{
    //This Function will search for the closest
    //value in the array that the user specified.
        ...
        for(int x = 0 ; x<m ; x++)
        {
            ...
            else if(arr[m-1]<searchval)
                {
                    return  m-1;
                }
             else if(arr[x]<searchval && arr[x+1]>searchval)
                {
                    cout<<x<<" and "<< x+1;
